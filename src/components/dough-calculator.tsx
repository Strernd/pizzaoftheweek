...
export default function DoughCalculator() {
  ...
  const [yeastType, setYeastType] = useState<"fresh" | "dry" | "instant">(
    "fresh"
  );
  ...
  useEffect(() => {
    try {
      ...
      const flourWeight = Math.round(
        (100 / totalPercentage) * targetTotalWeight
      );
      const waterWeight = Math.round(
        (waterPercentage / totalPercentage) * targetTotalWeight
      );
      const saltWeight = Math.round(
        (saltPercentage / totalPercentage) * targetTotalWeight
      );
      ...
      try {
        ...
        if (roomTempC < minTempC || roomTempC > maxTempC) {
          throw new Error(
            `Room temperature must be between ${minTempC}°C and ${maxTempC}°C`
          );
        }

        if (
          coldProofEnabled &&
          (coldTempC < minTempC || coldTempC > maxTempC)
        ) {
          throw new Error(
            `Cold temperature must be between ${minTempC}°C and ${maxTempC}°C`
          );
        }
        ...
        if (yeastPct < minYeastPct || yeastPct > maxYeastPct) {
          setCalculationWarning(
            `The calculated yeast percentage (${yeastPct.toFixed(
              3
            )}%) is outside the recommended range (${minYeastPct}% to ${maxYeastPct}%).`
          );
        }
        ...
        if (error instanceof Error) {
          ...
          if (error.message === "Temperature too low or duration too short") {
            ...
            setYeastError(
              "Fermentation time is too short. Please increase the room temperature proof time."
            );
      ...
      const adjustedFlourWeight = Math.round(
        flourWeight * (1 - reductionFactor)
      );
      const adjustedWaterWeight = Math.round(
        waterWeight * (1 - reductionFactor)
      );
      ...
      if (
        isNaN(finalFlourWeight) ||
        isNaN(adjustedWaterWeight) ||
        isNaN(adjustedSaltWeight) ||
        isNaN(yeastWeight)
      ) {
      ...
      const flourWeight = Math.round(
        (100 / totalPercentage) * targetTotalWeight * 0.98
      ); // 98% to leave room for yeast
      const waterWeight = Math.round(
        (waterPercentage / totalPercentage) * targetTotalWeight * 0.98
      );
      const saltWeight = Math.round(
        (saltPercentage / totalPercentage) * targetTotalWeight * 0.98
      );
      const yeastWeight =
        targetTotalWeight - flourWeight - waterWeight - saltWeight;
  ...
  return (
    <div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">
              Cold Proof (Refrigerator)
            </h3>
            <div className="flex items-center space-x-2">
              <Switch
                id="cold-proof"
                checked={coldProofEnabled}
                onCheckedChange={setColdProofEnabled}
              />
              <Label
                htmlFor="cold-proof"
      ...
      {yeastError && (
        <Alert
          variant="destructive"
          className="mb-6 border-red-500 dark:border-red-400 bg-red-50 dark:bg-red-950"
        >
          <AlertTriangle className="h-4 w-4 text-red-600 dark:text-red-300" />
          <AlertDescription className="font-medium text-red-600 dark:text-red-300">
            {yeastError}
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-3 gap-2">
          <Button
            ...
            className={
              yeastType === "fresh" ? "bg-red-500 hover:bg-red-600" : ""
            }
          >
            Fresh
          </Button>
          ...
          <Button
            ...
            className={
              yeastType === "instant" ? "bg-red-500 hover:bg-red-600" : ""
            }
          >
            Instant Dry
          </Button>
